# ?? Lista de Requisitos ¡ª Proyecto IndigoAssist
...
- **WEB-RF2. Gesti¨®n de Tickets.**  
  - CRUD: solicitante, categor¨ªa, prioridad, estado, t¨¦cnico, SLA, adjuntos.  
...
- **WEB-RF5. Gesti¨®n de Usuarios y Cat¨¢logos.**  
  Administraci¨®n de usuarios, roles, categor¨ªas y prioridades.

- **WEB-RF6. Reportes B¨¢sicos.**  
  Tickets por estado, t¨¦cnico o categor¨ªa; exportaci¨®n a CSV y PDF.
...
POST /v1/tickets ¡û creaci¨®n (m¨®vil usa usuario del token)
GET /v1/tickets/{id}
PATCH /v1/tickets/{id} ¡û cambiar estado/prioridad
POST /v1/tickets/{id}/asignar ¡û asignar t¨¦cnico
POST /v1/tickets/{id}/comentarios
POST /v1/tickets/{id}/adjuntos ¡û subida de archivos (retorna fileId)
...
POST /v1/files ¡û subida (con l¨ªmite de tama?o configurable)
GET /v1/files/{id} ¡û descarga autorizada con URL firmada temporal
...
El microservicio de notificaciones consume los eventos y env¨ªa:
- **Push:** FCM / APNS  
- **Email:** v¨ªa SMTP o Graph
...
- *Pendientes*
- *Cerrados*  
Acciones: cambiar estado, comentar, subir evidencia (c¨¢mara o galer¨ªa), registrar tiempo.
...
Formulario con usuario actual prellenado, categor¨ªa, prioridad, adjuntos y geolocalizaci¨®n opcional.

- **MOB-RF5. Activos con C¨®digo.**  
Escaneo QR/Code128 ¡ú detalle de activo ¡ú nueva intervenci¨®n o bit¨¢cora.
...
- **DB-RF2. ¨ªndices y B¨²squedas.**  
¨ªndices por Estado, T¨¦cnicoId, FechaCreacion y b¨²squeda textual en Asunto/Descripci¨®n.
...
Base de Datos: almacena tickets, usuarios, activos y registros de auditor¨ªa.
...
## ?? Contexto del Proyecto ¡ª IndigoAssist
...
Cada vez que se crea un ticket, se asigna un t¨¦cnico o se a?ade una anotaci¨®n, el sistema env¨ªa notificaciones por correo tanto al usuario que abri¨® el ticket como al t¨¦cnico asignado.  
...
- Registrar equipos, usuarios responsables y estados de garant¨ªa.  
...
- Permitir escalabilidad modular mediante microservicios (notificaciones, auditor¨ªa, SLA engine, etc.).
...
A la vez, se agregar¨¢n nuevas tablas dedicadas al m¨®dulo de activos y auditor¨ªas, sin alterar los procesos actuales.
...
optimizando la comunicaci¨®n, el control y la eficiencia operativa mediante el uso conjunto de tecnolog¨ªas **MVC, MAUI y API REST** sobre una **base de datos unificada y evolutiva**.



# ?? Historias de Usuario ¡ª IndigoAssist
...
- El sistema env¨ªa la solicitud a `/auth/token`.  
...
**Pasos:**
- El administrador accede al m¨®dulo **Usuarios ¡ú Crear nuevo**.  
...
- El usuario puede filtrar por rango de fechas, categor¨ªa o sucursal.
...
- Da clic en ¡°Tomar Ticket¡± ¡ú llamada a `/v1/tickets/{id}/asignar`.  
...
- El archivo se env¨ªa a `/v1/files` (verifica tipo y tama?o).  
...
**Pasos:**
- Acceder al m¨®dulo **Activos ¡ú Nuevo**.  
